...
export default function FacturasEmitidas() {
  ...
  async function abrirReimprimir(factura: any) {
    const { data, error } = await supabase
      ...
      .order("linea");
    if (error) {
      toast({ title: "Error cargando detalle", description: error.message, variant: "destructive" });
      return;
    }
    ...
    <div className="p-4 lg:p-8 max-w-6xl mx-auto space-y-6">
              ...
              {filtradas.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="text-center text-muted-foreground py-10">
                    {q ? "Sin resultados para esa búsqueda" : "Aún no hay facturas emitidas"}
                  </TableCell>
                </TableRow>
              ) : (
                filtradas.map((f: any) => (
                  <TableRow key={f.id}>
                    ...
                    <TableCell className="hidden sm:table-cell text-sm">{f.fecha_emision}</TableCell>
                    <TableCell className="text-sm max-w-[140px] truncate">{f.cliente_nombre}</TableCell>
                    <TableCell className="font-mono text-xs hidden md:table-cell">{f.cliente_rtn ?? "—"}</TableCell>
                    <TableCell className="text-right font-mono text-sm">{lempiras(parseFloat(f.total))}</TableCell>
                    <TableCell>
                      <Badge
                        variant={
                          f.estado === "anulada"
      ...
      <Dialog open={showAcciones} onOpenChange={setShowAcciones}>
        <DialogContent className="w-[95vw] sm:max-w-sm">
          <DialogHeader>
            <DialogTitle className="font-mono text-sm">{selected?.numero_factura}</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-muted-foreground mb-2">{selected?.cliente_nombre}</p>
        ...
        <DialogContent className="w-[98vw] max-w-[880px] max-h-[90vh] overflow-y-auto p-2 sm:p-6">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Printer className="w-5 h-5" /> Reimprimir — {selected?.numero_factura}
            </DialogTitle>
          </DialogHeader>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong>{selected?.cliente_nombre}</strong>. Esta acción no se puede deshacer.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {anulando ? <RefreshCw className="w-4 h-4 animate-spin mr-2" /> : <Ban className="w-4 h-4 mr-2" />}
              Anular
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
